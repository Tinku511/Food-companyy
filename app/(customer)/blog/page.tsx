  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 min-h-screen">
      <div className="text-center mb-16">
        <h1 className="font-display text-4xl font-bold text-foreground md:text-5xl">The Seseme Journal</h1>
      ...
      {posts.length === 0 ? (
        <div className="text-center py-20 text-stone-400">
          ...
          <p className="font-medium text-lg">No stories published yet.</p>
          <p className="text-sm mt-1">Check back soon for fresh content!</p>
        </div>
      ) : (
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm border border-stone-100 transition-all hover:shadow-md hover:-translate-y-1">
              <div className="aspect-[16/9] w-full overflow-hidden bg-stone-100 relative">
                ...
                <div className="absolute top-4 left-4 rounded-full bg-white/90 backdrop-blur-sm px-3 py-1 text-xs font-bold uppercase tracking-wider text-charcoal">
                  {new Date(post.publishedAt!).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </div>
              </div>
              
              <div className="flex flex-1 flex-col p-6">
                <h2 className="font-display text-xl font-bold text-foreground transition-colors group-hover:text-brass line-clamp-2">
                  {post.title}
                </h2>
                {post.excerpt && (
                  <p className="mt-3 flex-1 text-sm text-stone-500 line-clamp-3 leading-relaxed">
                    {post.excerpt}
                  </p>
                )}
