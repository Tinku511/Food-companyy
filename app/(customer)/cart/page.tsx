      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col lg:flex-row lg:divide-x lg:divide-stone-200">
        ...
        >
          ...
          <div className="mb-10 flex items-center justify-between">
            <div>
              <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-brass">Your Basket</p>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-5 py-24 text-center">
              ...
              <div className="max-w-xs">
                ...
                <p className="mb-6 text-stone-500">Looks like you haven&apos;t added anything yet.</p>
                <Link href="/products" className="btn-primary">Start Shopping</Link>
              </div>
            </div>
          ) : (
            <ul className="space-y-0 border-t border-stone-200 mt-8">
              {items.map((item) => (
                <li
                  key={item.id}
                  className="group flex items-center gap-6 border-b border-stone-200 py-6 bg-transparent"
                >
                  {/* Product image */}
                  ...
                  <div className="flex flex-1 flex-col gap-2 min-w-0">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                      <div className="inline-flex items-center rounded-full border border-stone-200 bg-background">
                        ...
                        <span className="w-9 text-center text-sm font-semibold text-foreground">{item.quantity}</span>
                        <button
                          type="button"
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
        ...
        {items.length > 0 && (
          <div className="w-full border-t border-stone-200 bg-background/70 px-4 pb-12 pt-8 lg:w-96 lg:border-t-0 lg:px-8 lg:pt-24">
            <div className="sticky top-24">
              ...
              <div className="flex items-end justify-between py-6">
                <span className="text-sm font-semibold uppercase tracking-widest text-stone-400">Total</span>
                <span className="font-display text-4xl font-bold text-charcoal">₹{cartTotal.toFixed(2)}</span>
              </div>

              {/* CTA */}
              <Link
                href="/checkout"
                className="btn-primary w-full shadow-brand-md"
              >
