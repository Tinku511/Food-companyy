...
export const metadata: Metadata = {
  ...
  description: 'Browse our full range of natural, farm-fresh food products. Filter by category to find snacks, beverages, dairy, and bakery items.',
};
  ...
  return (
    <div className="animate-pulse overflow-hidden rounded-2xl border border-stone-100 bg-white shadow-card">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="mt-2 skeleton h-5 w-1/3 rounded" />
      </div>
    </div>
  );
}
    ...
    return (
      <div className="col-span-full flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-stone-200 py-28 text-center">
        ...
        <p className="mt-2 text-sm text-stone-500">
          {category ? `No "${category}" products available right now. Try a different category.` : 'No products available yet.'}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
      {products.map((product, i) => (
        <div key={product.id} className={`animate-slide-up`} style={{ animationDelay: `${i * 60}ms` }}>
          <ProductCard product={{ ...product, price: Number(product.price) } as any} priority={i < 3} />
        </div>
      ))}
    </div>
  );
      ...
      <div className="bg-hero-gradient">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <h1 className="text-4xl font-bold text-white sm:text-5xl">
            {category ? (
              <>
                <span className="text-stone-400 text-2xl sm:text-3xl font-medium block mb-1">Category</span>
                {category}
              </>
            ) : (
              'All Products'
